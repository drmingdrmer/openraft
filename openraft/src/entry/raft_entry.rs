    ...
    Self: RaftPayload<P>,
{
    ...
    fn new_blank(log_id: LogIdOf<P>) -> Self
    ...
    #[since(version = "0.10.0", change = "become a default method")]
    fn new_normal(log_id: LogIdOf<P>, data: P::D) -> Self
    where Self: Final + Sized {
    ...
    #[since(version = "0.10.0", change = "become a default method")]
    fn new_membership(log_id: LogIdOf<P>, m: Membership<P>) -> Self
    where Self: Final + Sized {
    ...
    #[since(version = "0.10.0")]
    fn log_id(&self) -> LogIdOf<P>
    where Self: Final {
        let (leader_id, index) = self.log_id_parts();
        LogIdOf::<P>::new(leader_id.clone(), index)
    }

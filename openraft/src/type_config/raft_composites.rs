...
use crate::type_config::alias::DOf;
    ...
    Sized + OptionalSend + OptionalSync + Debug + Clone + Copy + Default + Eq + PartialEq + Ord + PartialOrd + 'static
{
    ...
    /// Raft vote type.
    type Vote: RaftVote<LeaderId = <Self::Prim as RaftPrimitives>::LeaderId>;

    /// Raft log entry type.
    type Entry: RaftEntry<
            CommittedLeaderId = CommittedLeaderIdOf<Self::Prim>,
            D = DOf<Self::Prim>,
            NodeId = NodeIdOf<Self::Prim>,
            Node = NodeOf<Self::Prim>,
        >;

    ...
    Self: OptionalFeatures + Eq + Clone + Debug + Display + 'static,
{
    ...
    ///
    /// This creates an owned [`Vote<P>`] from any vote implementation.
    ...
    #[since(version = "0.10.0")]
    fn partial_cmp<V: RaftVote<P>>(&self, other: &V) -> Option<Ordering> {
        let self_ref: RefVote<'_, P> = RefVote::new(self.leader_id(), self.is_committed());
        let other_ref: RefVote<'_, P> = RefVote::new(other.leader_id(), other.is_committed());
        PartialOrd::partial_cmp(&self_ref, &other_ref)
    }
}

pub(crate) trait RaftVoteExt<P>
where
    P: RaftPrimitives,
    Self: RaftVote<P>,
{
    #[allow(dead_code)]
    fn new_with_default_term(node_id: P::NodeId) -> Self {
        let leader_id = P::LeaderId::new_with_default_term(node_id);
        Self::from_leader_id(leader_id, false)
    }

    /// Creates a new vote for a node in a specific term, with uncommitted status.
    fn from_term_node_id(term: P::Term, node_id: P::NodeId) -> Self {
        let leader_id = P::LeaderId::new(term, node_id);
        Self::from_leader_id(leader_id, false)
    }

    fn from_term_node_id_committed(term: P::Term, node_id: P::NodeId, committed: bool) -> Self {
        let leader_id = P::LeaderId::new(term, node_id);
    ...
    #[allow(dead_code)]
    fn try_to_committed(&self) -> Option<CommittedVote<P>> {
        if self.is_committed() {
    ...
    #[allow(dead_code)]
    fn try_to_committed_leader_id(&self) -> Option<CommittedLeaderIdOf<P>> {
        if self.is_committed() {
    ...
    fn is_same_leader(&self, leader_id: &CommittedLeaderIdOf<P>) -> bool {
        self.leader_id().to_committed() == *leader_id
    }
}

impl<P, T> RaftVoteExt<P> for T
where
    P: RaftPrimitives,
    T: RaftVote<P>,
{

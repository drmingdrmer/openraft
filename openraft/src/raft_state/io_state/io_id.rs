...
where P: RaftPrimitives
{
    pub(crate) fn new_vote_io(vote: UncommittedVoteOf<P>) -> Self {
        Self::Vote(vote)
    }

    pub(crate) fn new_log_io(committed_vote: CommittedVoteOf<P>, last_log_id: Option<LogIdOf<P>>) -> Self {
        Self::Log(LogIOId::new(committed_vote, last_log_id))
    }

    /// Unpack into internal vote and last log id for progress tracking.
    ///
    /// Returns the concrete `Vote<P::LeaderId>` type (not trait `VoteOf<C>`) because
    /// progress tracking requires `PartialOrd`, which user-defined `VoteOf<C>`
    /// may not implement.
    pub(crate) fn to_vote_and_log_id(&self) -> (Vote<P::LeaderId>, Option<LogIdOf<P>>) {
        match self {
    ...
    pub(crate) fn as_ref_vote(&self) -> RefVote<'_, P::LeaderId> {
        match self {
    ...
    pub(crate) fn to_committed_vote(&self) -> Option<CommittedVoteOf<P>> {
        match self {
            Self::Vote(_) => None,

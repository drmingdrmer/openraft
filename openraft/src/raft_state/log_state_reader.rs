...
pub(crate) trait LogStateReader<P>
where P: RaftPrimitives
{
    /// Get previous log id, i.e., the log id at index - 1
    fn prev_log_id(&self, index: u64) -> Option<LogIdOf<P>> {
    ...
    fn has_log_id(&self, log_id: impl RaftLogId<P>) -> bool {
    ...
    fn get_log_id(&self, index: u64) -> Option<LogIdOf<P>> {
    ...
    fn io_applied(&self) -> Option<&LogIdOf<P>>;
    ...
    fn io_snapshot_last_log_id(&self) -> Option<&LogIdOf<P>>;

    /// The last known purged log id, inclusive.
    ///
    /// This is actually purged log id from storage.
    fn io_purged(&self) -> Option<&LogIdOf<P>>;

    /// Return the last log id the snapshot includes.
    fn snapshot_last_log_id(&self) -> Option<&LogIdOf<P>>;
    ...
    fn purge_upto(&self) -> Option<&LogIdOf<P>>;
    ...
    fn last_purged_log_id(&self) -> Option<&LogIdOf<P>>;
}

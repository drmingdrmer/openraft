...
where P: RaftPrimitives
{
    ...
    fn sto_write_snapshot(self, signature: Option<SnapshotSignatureOf<P>>) -> Result<T, StorageError<P>>;

    /// Convert io::Error to StorageError for reading a snapshot
    fn sto_read_snapshot(self, signature: Option<SnapshotSignatureOf<P>>) -> Result<T, StorageError<P>>;

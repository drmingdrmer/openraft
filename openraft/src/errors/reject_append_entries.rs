...
pub(crate) enum RejectAppendEntries<C: RaftTypes> {
...
impl<C: RaftTypes> From<RejectVote<C>> for RejectAppendEntries<C> {
    fn from(r: RejectVote<C>) -> Self {
        RejectAppendEntries::RejectVote(r)
    }
}

impl<C: RaftTypes> From<RejectAppendEntries<C>> for StreamAppendError<C> {
...
impl<C: RaftTypes> Peel for RejectAppendEntries<C> {

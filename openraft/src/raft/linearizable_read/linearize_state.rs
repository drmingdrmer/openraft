...
#[derive(Debug, Clone)]
pub struct LinearizeState<P>
where P: RaftPrimitives
{
    /// The node from which this Linearizer collects the applied log ID.
    node_id: P::NodeId,
    read_log_id: LogIdOf<P>,
    applied: Option<LogIdOf<P>>,
}

impl<P> fmt::Display for LinearizeState<P>
where P: RaftPrimitives
{
...
impl<P> LinearizeState<P>
where P: RaftPrimitives
{
    pub(crate) fn new(node_id: P::NodeId, read_log_id: LogIdOf<P>, applied: Option<LogIdOf<P>>) -> Self {
        Self {
            node_id,
            read_log_id,
            applied,
    ...
    pub(crate) fn with_applied(mut self, node_id: P::NodeId, applied: Option<LogIdOf<P>>) -> Self {
    ...
    pub(crate) fn is_ready_on_node(&self, node_id: &P::NodeId) -> bool {
    ...
    #[since(version = "0.10.0")]
    pub fn read_log_id(&self) -> &LogIdOf<P> {
        &self.read_log_id
    }

    /// The last applied log ID.
    #[since(version = "0.10.0")]
    pub fn applied(&self) -> Option<&LogIdOf<P>> {
        self.applied.as_ref()
    }
}

...
where P: RaftPrimitives
{
    pub(crate) fn new(node_id: P::NodeId, read_log_id: LogId<P>, applied: Option<LogId<P>>) -> Self {
        Self {
            node_id,
            read_log_id,
            applied,
    ...
    pub(crate) fn with_applied(mut self, node_id: P::NodeId, applied: Option<LogId<P>>) -> Self {
    ...
    #[since(version = "0.10.0")]
    pub fn read_log_id(&self) -> &LogId<P> {
        &self.read_log_id
    }

    /// The last applied log ID.
    #[since(version = "0.10.0")]
    pub fn applied(&self) -> Option<&LogId<P>> {
        self.applied.as_ref()
    }
}

...
async fn join_promotes_to_voter_and_cluster_survives_leader_death() -> io::Result<()> {
    ...
    // A direct read serves the serialized app state from local memory - no
    // consensus round, no log entry.
    let state: serde_json::Value = reqwest::get(format!("http://{}/api/read", addr_a))
        .await
        .map_err(io::Error::other)?
        .json()
        .await
        .map_err(io::Error::other)?;
    assert_eq!(serde_json::json!({"data": {"k1": "v1"}}), state);

...
where T: EzApp
{
    ...
    ///
    /// Serves the whole application state, serialized, straight from local memory: no consensus
    /// round and no log entry. The answer is as fresh as this node's replication - a read that
    /// must be linearizable goes through [`EzRaft::write`] instead.
    async fn handle_read(ez: Data<Self>) -> Result<web::Json<serde_json::Value>, actix_web::Error> {
        let state = ez
            .raft
            .read(|app| serde_json::to_value(app))
            .await
            .map_err(|e| actix_web::error::ErrorInternalServerError(format!("read failed: {}", e)))?;

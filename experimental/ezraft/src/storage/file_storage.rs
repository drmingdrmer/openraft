...
///
/// - **Durability.** Writes are not `fsync`ed, so a machine crash can lose a vote or a log
///   entry this node already acknowledged - the one assumption [`EzStorage`] cannot do without.
///   Anything beyond a demo needs storage that returns from `persist` only once the bytes would
